...
// ! Function under test
function sanitizeFruitBasket(fruits, unwantedFruit) {
  return fruits.filter((basketItem) => basketItem !== unwantedFruit);
}
...
describe('sanitizeFruitBasket', () => {
  ...
  test('should not modify the original `fruitBasket` array', () => {
    ...
    expect(JSON.stringify(fruitBasket)).toBe(
      JSON.stringify(originalFruitBasketContents)
    );
  });

  test('should return a new array that does not include the unwanted `lemon`', () => {
    const result = JSON.stringify(sanitizeFruitBasket(fruitBasket, 'lemon'));
